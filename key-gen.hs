...
keyGen = do
	pubKey <- openFile "rsa-pub.txt" WriteMode
	prvKey <- openFile "rsa-prv.txt" WriteMode
	randGen <- getStdGen
...
primeList gen = filter (isRabinMillerPrime gen) randomList
	where randomList = randomRs ( (2^100), (2^112) ) gen
...
powerMod b e m 
	| e == 0 = 1
	| (odd e) = ( b * (powerMod (t) (e `div` 2) m)) `mod` m 
	| otherwise = (powerMod (t) (e `div` 2) m)
	where t = (b * b) `mod` m